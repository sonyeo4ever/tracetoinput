...
//		printf("%s\n", line);
		...
		else if (format_type == TYPE_RENAME)
		{
			...
			if (ptr == NULL)
				continue;
			strncpy(path_old, ptr, strlen(ptr));
			path_old[strlen(ptr)] = 0x00;
		...
		if (format_type != TYPE_WRITE)
		{
			...
			else if (strstr(line, "[UN]"))
			{
				struct Namemap *found_entry = *(struct Namemap**) searchMap;
				found_entry->delete = 1;
				free(entry);
			}
			else {
				struct Namemap *found_entry = *(struct Namemap**) searchMap;
				setname(found_entry, path);
				found_entry->delete = 0;
				free(entry);
			}
		}
		else
		{
			...
			if (searchMap == NULL) {
				...
				strncpy(path, result_entry->name.name, strlen(result_entry->name.name));
				path[strlen(result_entry->name.name)] = 0x00;
...
struct Namemap* make_namemap_entry(long long int inode)
{
	...
	new_namemap->delete = 0;
}

int setname(struct Namemap* entry, char *path)
{
	memset(entry->name.name, 0, PATH_MAX + 1);
	strncpy(entry->name.name, path, strlen(path));
	entry->name.name[strlen(path)] = 0x00;
	entry->delete = 0;
}
