...
void const walk_user(void *nodep, const VISIT which, const int depth)
{
	const struct Namemap_p *namemap = *(const struct Namemap_p**) nodep;
...
static int get_ps(char* file, void **name_tree)
{
	char line[2048];
	int ret;
	FILE *preopen_fp;
	...
	while (fgets(line, 2048, preopen_fp) != NULL)
	{
		...
		entry = make_namemap_entry_p(user);
		searchMap = tfind(entry, name_tree, compare_user);
		if (searchMap == NULL)
		{
		...
		int flag, struct FTW *ftwbuf)
{
	...
	void *name_tree = NULL, *p_tree = NULL;
	char *tmp;
	int p_cnt = 0, i;
